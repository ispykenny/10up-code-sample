...
const title = 'Movie Review';

const LogoSvg = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 235.84 269.94">
      <g>
        <g>
          <path
...
export const Logo = ({ includeDisplayName, asLink }: LogoProps) => {
  return (
    <>
      {asLink ? (
        <Link className="logo" href="/" aria-label="Home">
          <LogoSvg />
          {includeDisplayName && <span>{title}</span>}
        </Link>
      ) : (
        <span className="logo">
          <LogoSvg />
